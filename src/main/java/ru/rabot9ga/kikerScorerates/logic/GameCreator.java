...
public class GameCreator {
    ...
    private CreateGameRs isValid(CreateGameRq createGameRq) {
        ...
        for (Team team : teams) {
            if (team.getPlayers() == null || team.getPlayers().size() < 1 || team.getPlayers().size() > 2) {
                return new CreateGameRs().setUuid(createGameRq.getUuid())
                        .setStatus(Status.ERROR)
    ...
    private void createEmptyPlayers() {
        ...
        createGameRq.getTeams().stream().map(Team::getPlayers).forEach(players::addAll);
        ...
        if (nullTeam.size() > 0) {
            ...
            List<MongoTeam> teamToCreate = nullTeam.stream()
                    ...
                    .map(team -> new MongoTeam()
                            ...
                            .setPlayer(mapIDMongoPlayer.get(team.getPlayers().get(0).getId()))
                            .setPlayer(mapIDMongoPlayer.get(team.getPlayers().get(1).getId()))
                    )
